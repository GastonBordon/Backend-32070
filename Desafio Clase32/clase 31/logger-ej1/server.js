const express = require("express");
const logger = require("./logger");

const app = express();

// const data = ('I love GeeksforGeeks').repeat(8000)

app.get("/sumar", (req, res) => {
  const { a, b } = req.query;
  const resultado = parseInt(a) + parseInt(b);
  if (isNaN(a) || isNaN(n)) {
    logger.error("Se ingresó un valor no numérico");
    res.status(400).send("Se ingresó un valor no numérico");
  }

  logger.info(`Se sumaron ${a} y ${b} y el resultado es ${resultado}`);
  res.send(`El resultado de la suma es ${resultado}`);
});

app.get("*", (req, res) => {
  const { url, method } = req;
  logger.info(`Se recibió una petición ${method} a la ruta ${url}`);
  res.send("No existe la ruta");
});

const server = app.listen(4000, (err) => {
  if (err) {
    logger.error("Error al iniciar Servidor");
  }
  logger.info("Server is running on port 4000");
});

server.on("error", (err) => {
  logger.error("Error en el servidor");
});

server.on("error", (err) => {});
